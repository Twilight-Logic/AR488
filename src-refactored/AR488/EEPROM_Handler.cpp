...
/*
void eepromHandler::printHelpInfo(const __FlashStringHelper * token, const __FlashStringHelper * desc) {
  dataPort.print(F("++"));
 ...
 */
/* 
void eepromHandler::epWriteData(uint8_t cfgdata[], size_t cfgsize) {
  uint16_t crc;
 
  // Write data
  for (size_t i=0; i<cfgsize; i++){
    EEPROM.update(i+EESTART,cfgdata[i]);
  }
  // Write CRC
...
/*
 * EESTART = starting EEPROM address for configuration data
 * cfg = config data union object
 * csize = size of config data object
 */
/*
bool eepromHandler::epReadData(uint8_t cfgdata[], size_t cfgsize) {
  uint16_t crc1;
  uint16_t crc2;
...
/***** Generate a CRC *****/
/*
unsigned long int eepromHandler::getCRC32(uint8_t bytes[], size_t bsize) {
...
uint16_t eepromHandler::getCRC16(uint8_t bytes[], size_t bsize){
  uint8_t x;
  uint16_t crc = 0xFFFF;
