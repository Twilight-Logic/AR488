...
/***** FWVER "AR488 GPIB controller, ver. 0.49.01, 06/07/2020" *****/
...
/*
 * At present requires no more than 128 bytes 
 * (Planned to have 4 x 128 byte 'pages' or  
 * 2 x 256 byte 'pages' to support 4 possible 
...
void addr_h(char *params) {
  ...
  if (params != NULL) {
    ...
    if (param != NULL) {
      if (notInRange(param, 1, 30, val)) {
        if (notInRange(param, 96, 126, val)) {
          errBadCmd();
          return;
        }
      }
      if (val>95) val - GC_SAD;
      AR488.saddr = val;
      if (isVerb) {
    ...
    if (AR488.saddr > 0) {
      arSerial->print(F(" "));
      arSerial->print(AR488.saddr+GC_SAD);
    }
    arSerial->println();
  }
}
...
void tmbus_h(char *params) {
  ...
  if (params != NULL) {
//    val = atoi(params);
    if (notInRange(params, 0, 30000, val)) {
      errBadCmd();
      return;
    }
    AR488.tmbus = val;
...
bool addrDev(uint8_t addr, bool dir) {
  ...
  if (dir) {
    ...
    if (AR488.saddr>0) { if (gpibSendCmd(GC_SAD + AR488.saddr)) return ERR; };
