 ...
 *
 * Mofified for Tom (iphi), based on version 0.04.06
 */
...
#define VERSION "0.04.66" // 06-03-2021
...
ESP8266WebServer *AR488srv = new ESP8266WebServer(80);
...
Serial.print(F("Web port: "));Serial.println(AP.webp);
Serial.print(F("GPIB port: "));Serial.println(AP.gpibp);
Serial.print(F("SSL enabled: "));Serial.println(AP.ssl);
Serial.print(F("GPIB pass en.: "));Serial.println(AP.gpib);
Serial.print(F("Client: "));Serial.println(AP.wclient);
Serial.print(F("DHCP: "));Serial.println(AP.dhcp);
Serial.print(F("IP addr1: "));Serial.println(AP.addr[0]);
Serial.print(F("IP addr2: "));Serial.println(AP.addr[1]);
Serial.print(F("IP addr3: "));Serial.println(AP.addr[2]);
Serial.print(F("IP addr4: "));Serial.println(AP.addr[3]);
*/
...
/* We don't need the AR488 config page  
...
/* We don't need the AR488 config javascript  
...
/* We don't need the AR488 config actions
...
/*
 * We don't need to get the version number from the AR488
 * 
...
<!-- AR488 config page not required\
...
<!-- AR488 version information not required\
...
-->\
           ...
           curPage,
           VERSION  //, comma removed
/* AR488 firmware version not required\
           fwver\
*/           
          );
...
*/
/*
 * We don't need the configruation page for the AR488 GPIB adapter
...
/*
 * we don't need the JavaScript that makes the AR488
 * config page work
...
/*
 * We no longer need the code that makes the AR488 configuration page work
...
void epViewData() {
  ...
  for (addr=0; addr<512; addr=addr+16){
    ...
    printHex(dbuf, 16, UCASE);
  }
    EEPROM.end();
}


void printHex(uint8_t *data, uint8_t dlen, bool ul) {
  char tmp[dlen*3+1];
  byte db;
  int j=0;
  uint8_t offs;

  // Upper/lowercase charater offset
  offs = ul ? 7 : 39;
  // Process bytes
  for (uint8_t i=0; i<dlen; i++) {
    db = (data[i] >> 4) | 48;
    tmp[j] = (db > 57) ? (db + (byte)offs) : db;
    j++;
    db = (data[i] & 0x0F) | 48;
    tmp[j] = (db > 57) ? (db + (byte)offs) : db;
    j++;
